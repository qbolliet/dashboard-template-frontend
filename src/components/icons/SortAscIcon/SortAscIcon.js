'use client';

import React from 'react';

/**
 * Sort-ascending icon component.
 * Chevron-up + vertical bar — column header state when sorted ascending.
 *
 * @param {string} className - Additional CSS classes.
 * @param {number} width - Icon width (default: 16, usually overridden by CSS).
 * @param {number} height - Icon height (default: 16, usually overridden by CSS).
 * @param {Object} props - Other props forwarded to the SVG.
 * @returns {JSX.Element} The rendered sort-ascending icon SVG.
 */
const SortAscIcon = ({
    className = '',
    width = 16,
    height = 16,
    ...props
}) => {
    return (
        <svg
            width={width}
            height={height}
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
            xmlns="http://www.w3.org/2000/svg"
            className={`sort-asc-icon ${className}`}
            aria-hidden="true"
            {...props}
        >
            <path d="M7 9l5-5 5 5" />
            <line x1="12" y1="4" x2="12" y2="20" />
        </svg>
    );
};

export default SortAscIcon;
